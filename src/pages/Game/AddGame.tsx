...
import { useDefineGameMutation } from "../../services/gameApi";
...
const schema = yup.object().shape({
	...
	media: yup
		.array()
		.required("Foto permainan harus diisi")
...
function AddGame() {
	...
	const [defineGame, { isLoading }] = useDefineGameMutation();
	...
	const onSubmit: SubmitHandler<GameAddRequest> = async (data) => {
		try {
			await defineGame(data).unwrap();
			dispatch(setAllowedToast());
			showSuccessToast("Data permainan berhasil ditambahkan");
			navigate("/game");
		} catch (error) {
			showErrorToast("Gagal menambahkan data permainan");
		}
	};
		...
		<div>
			<div className="mb-6">
				...
				<div className="flex items-center justify-between">
					<div className="">
						<h5 className="font-semibold text-3xl mb-1.5">Tambah Game</h5>
						<p className="text-gray-500">
							Tambahkan permainan baru ke dalam sistem.
						</p>
					</div>
